		...
		t.Run(name, func(t *testing.T) {
			...
			t.Run("Add template repo", func(t *testing.T) {
				...
				if test.inGitCredentials != nil {
					...
					savedGitCredentials, keyringErr := security.GetSecretFromKeyring(cwTest.ConID, "gitcredentials-"+IDOfAddedRepo)
					assert.Nil(t, keyringErr)

					var gitCredentialsStruct *utils.GitCredentials
					unmarshalErr := json.Unmarshal([]byte(savedGitCredentials), &gitCredentialsStruct)
					assert.Nil(t, unmarshalErr)
					assert.Equal(t, test.inGitCredentials, gitCredentialsStruct)
				}
			})
			...
			t.Run("Create project from template from added repo", func(t *testing.T) {
				...
				gitCredentials, err := GetGitCredentialsFromKeychain(cwTest.ConID, templateURL)
				assert.Nil(t, err)
				assert.NotNil(t, gitCredentials)

				result, err := project.DownloadTemplate(testDir, templateURL, gitCredentials)
				assert.Nil(t, err)
				if result != nil {
			...
			t.Run("Delete template repo", func(t *testing.T) {
				...
				if test.inGitCredentials != nil {
					savedGitCredentials, err := security.GetSecretFromKeyring(cwTest.ConID, "gitcredentials-"+IDOfAddedRepo)
					assert.Equal(t, "", savedGitCredentials)
